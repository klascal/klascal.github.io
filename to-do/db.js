...
const DEFAULT_LIST_NAME = "Huiswerk";
...
function loadSQLite() {
  if (!sqliteLoader) {
    sqliteLoader = new Promise((resolve, reject) => {
      ...
      script.setAttribute(
        "src",
        "./sqlite-wasm-3400000/jswasm/sqlite3-worker1-promiser.js"
      );
...
export async function getTodaysTasks() {
  ...
  const tasks = await sqlitePromiser("exec", {
    sql: `
      SELECT tasks.*, lists.title as list_title, lists.color as list_color FROM tasks
      LEFT JOIN lists ON tasks.list_id = lists.id
      WHERE tasks.completed = 0 AND tasks.due_by > 0 AND tasks.due_by >= ${start} AND tasks.due_by < ${
      start + 86400000
    };
    `,
    resultRows: [],
