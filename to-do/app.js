...
const moreListActionsDialog = document.querySelector(
...
const LIST_COLORS = [
  ...moreListActionsDialog.querySelectorAll(".list-colors input"),
...
async function selectList(listItem, highlightedTaskId) {
  moreListActionsButton.style.display = isListEditable(listItem.dataset.id)
    ? "block"
    : "none";
  editListTitleInput.disabled = !isListEditable(listItem.dataset.id);
  newTaskForm.style.display = isListEditable(listItem.dataset.id)
    ? "flex"
    : "none";
...
newTaskForm.onsubmit = async function (event) {
  ...
  selectedList.querySelector(".number-of-tasks").innerText =
    tasksElement.querySelectorAll(".task").length;
};
...
async function updateAutoListsCount() {
  ...
  document.querySelector(
    '.list[data-id="overdue"] .number-of-tasks'
  ).innerText = overdueCount;
  const todayCount = (await db.getTodaysTasks()).length;
  document.querySelector('.list[data-id="today"] .number-of-tasks').innerText =
    todayCount;
...
addEventListener("mousedown", (event) => {
  if (
    document.documentElement.classList.contains("is-editing") &&
...
async function initAutoLists() {
  ...
  const overdueListItem = createListItem(
    "overdue",
    "Deadline geweest",
    "coral",
    overdueTasks.length
  ...
  const dueTodayListItem = createListItem(
    "today",
    "Vandaag",
    "gainsboro",
