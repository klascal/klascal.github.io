...
var primaryAccent = getComputedStyle(document.body)
...
function displaySchedule(scheduleData) {
  ...
  const appointmentsByDay = appointments.reduce((acc, appointment) => {
    ...
    var currentDate = new Date(appointment.start * 1000);
      ...
      const appointmentsHTML = appointments
        .map((appointment) => {
          ...
          var startTime = new Date(appointment.start * 1000)
            ...
            .replace(/^0+/, "");
          var endTime = new Date(appointment.end * 1000)
            .toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          ...
          var subjects = appointment.subjects;
          ...
          var teachers =
            ...
            ")";
          if (localStorage.getItem("hoofdletter") == "true") {
            var teachers = teachers.toUpperCase();
          }
          if (
            window.innerWidth < 676 &&
            localStorage.getItem("dag") != "true"
          ...
          return `<div style="${positie}"
            ...
            teachers == "()" ? "" : teachers
                ...
                <div class="warning" style="${bottom}">
                  ${warningsymbol}
                  <span class="warningMessage" style="${right}">${warning}</span>
                </div>
              </span>
              <p class="className">
                ${appointment.groups.join(", ")}
...
async function handleFormSubmit() {
  ...
  document.getElementById("week").innerText = "Week " + week;
  if (week < 10) week = `0${week}`; // Voeg een voorloopnul toe aan enkelcijferige weken
  var firstDigit = startTime.value.split(":")[0].replace(/^0+/, "");
  var decimalStartTime = firstDigit + "." + startTime.value.split(":")[1] / 60;
