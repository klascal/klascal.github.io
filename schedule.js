...
function displaySchedule(scheduleData) {
	...
	if (localStorage.getItem("dag") === "true") {
		...
		document.getElementById("schedule").style =
			`width:500vw;transform:translateX(${transform}vw);transition:none;`;
		setTimeout(() => {
			document.getElementById("schedule").style =
				`width:500vw;transform:translateX(${transform}vw);`;
		}, 100);
		let resizeTimeout;
		...
		.map(([dayName, appointments]) => {
			const appointmentsHTML = appointments
				.map((appointment) => {
					...
					let teachers = `(${appointment.teachers.filter((e) => e !== user).join(", ")})`;
					if (localStorage.getItem("hoofdletter") === "true") {
